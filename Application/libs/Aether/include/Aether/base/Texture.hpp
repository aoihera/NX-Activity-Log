...
namespace Aether {
    ...
    class Texture : public Element {
        private:
            ...
            /**
             * @brief Mutex protecting tmpDrawable and the cancelled flag.
             * Held by destroy() and by RenderJob::work() so that destroy()
             * cannot free the Texture while the worker is still writing to it.
             */
            std::mutex renderMutex;

            /**
             * @brief Set to true by destroy() under renderMutex.
             * RenderJob::work() checks this after acquiring renderMutex; if set,
             * it discards its result instead of writing to tmpDrawable.
             */
            bool cancelled;
