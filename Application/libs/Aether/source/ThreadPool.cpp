...
namespace Aether {
    ...
    void ThreadPool::removeOrWaitForJob(int id) {
        // Check if job is still on the queue (not yet picked up by a worker).
        {
            std::scoped_lock<std::mutex> mtx(this->jobsMutex);
        ...
        // jobsMutex is now released before we wait on a worker CV.
        // Holding jobsMutex while blocking on a worker CV caused a deadlock:
        // workers need jobsMutex to dequeue the next job after finishing one.

        // Otherwise check if currently running on a worker thread.
        for (size_t i = 0; i < this->workerMetadata.size(); i++) {
            ...
            // Wait for thread to finish (jobsMutex is NOT held here).
