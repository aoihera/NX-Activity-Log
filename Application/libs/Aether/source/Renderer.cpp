...
namespace Aether {
    ...
    void Renderer::setFont(const std::string & path) {
        ...
        // Hold ttfMtx while clearing the cache so a worker thread cannot be
        // mid-blit on a glyph surface that setCustomFont() is about to free.
    ...
    std::vector<Colour> Renderer::readTexturePixels(SDL_Texture * texture) {
        ...
        //   1. Pass `tmp` (the allocated buffer) to SDL_RenderReadPixels, not
        //      &pixels[0] (pixels is empty, so &pixels[0] is UB).
        //   2. Free `tmp` on the early-return error path to avoid a leak.
        //   3. Use delete[] (not delete) for an array allocation.
        //   4. Index pixel channels correctly with stride i*4+channel.
        uint8_t * tmp = new uint8_t[w * h * 4];
        if (SDL_RenderReadPixels(this->renderer, nullptr, SDL_PIXELFORMAT_RGBA32, tmp, w * 4) < 0) {
        ...
        for (size_t i = 0; i < static_cast<size_t>(w * h); i++) {
            pixels.push_back(Colour(tmp[i*4], tmp[i*4+1], tmp[i*4+2], tmp[i*4+3]));
        }
        delete[] tmp;
    ...
    Drawable * Renderer::renderTextSurface(const std::string str, const unsigned int size) {
        ...
        if (surf == nullptr) {
            this->logMessage(std::string("Couldn't create surface for text: ") + std::string(SDL_GetError()), true);
            return new Drawable();
        }
        ...
        while (pos < str.length()) {
            ...
            // Glyph surfaces are owned by the font cache and shared across calls.
            // SDL_BlitSurface writes into the SOURCE surface's internal BlitMap
            // (src->map->sw_data) to cache pixel-format conversion state.  On the
            // Nintendo Switch, newlib's free() does NOT tolerate a NULL argument,
            // so when SDL calls SDL_InvalidateMap() on a glyph whose sw_data was
            // never initialised (same-format blit, no conversion table allocated)
            // it passes NULL to SDL_free() and crashes.  Fix: convert the cached
            // glyph to a private RGBA32 surface that has its own, independent map
            // state.  The copy is blitted from and then immediately freed, so the
            // shared cached surface is never mutated by SDL's blit machinery.
            {
                std::scoped_lock<std::mutex> mtx(this->ttfMtx);
                SDL_Surface * cached = this->fontCache->getGlyph(ch, size);
                if (cached == nullptr) {
                ...
                // Make a private copy so SDL_BlitSurface only touches our own surface.
                SDL_Surface * tmp = SDL_ConvertSurfaceFormat(cached, SDL_PIXELFORMAT_RGBA32, 0);
                if (tmp == nullptr) {
                    this->logMessage(std::string("Couldn't convert glyph surface: ") + std::string(SDL_GetError()), true);
                    this->destroySurface(surf, false);
                    return new Drawable();
                }
    ...
    Drawable * Renderer::renderWrappedTextSurface(const std::string str, const unsigned int size, const unsigned int width) {
        ...
        for (size_t i = 0; i < lines.size(); i++) {
            ...
            while (pos < lines[i].length()) {
                ...
                // Same fix as renderTextSurface: convert the cached (shared) glyph
                // surface to a private RGBA32 copy so SDL_BlitSurface cannot corrupt
                // the cached surface's BlitMap and cannot pass NULL to SDL_free().
                std::scoped_lock<std::mutex> mtx(this->ttfMtx);
                SDL_Surface * cached = this->fontCache->getGlyph(ch, size);
                if (cached == nullptr) {
                    this->logMessage("Couldn't get surface for glyph, is a font set?", true);
                ...
                SDL_Surface * glyph = SDL_ConvertSurfaceFormat(cached, SDL_PIXELFORMAT_RGBA32, 0);
                if (glyph == nullptr) {
                    this->logMessage(std::string("Couldn't convert glyph surface: ") + std::string(SDL_GetError()), true);
                    this->destroySurface(surf, false);
                    return new Drawable();
