...
namespace Aether {
                ...
                if (!(this->focused()->y() >= this->y() && this->focused()->y() + this->focused()->h() <= this->y() + this->h())) {
                    for (size_t i = 0; i < this->children.size(); i++) {
                        if (this->children[i]->y() > this->y() && this->children[i]->selectable()) {
                            ...
                            // FIX: guard against null parent() during screen transitions
                            if (this->parent() != nullptr && this->parent()->focused() == this) {
        ...
        if (it != this->children.end()) {
            ...
            if (this->focussed() == this->children[d]) {
                ...
                for (size_t i = 1; i < this->children.size(); i++) {
                    ...
                    } else if (d >= (long)i && (d-i) < (long)this->children.size()) {
                        if (this->children[d-i]->selectable()) {
                            this->setFocussed(this->children[d-i]);
                            break;
                        }
    ...
    bool Scrollable::removeElementsBefore(Element * e) {
        ...
        if (it != this->children.end()) {
            ...
            // FIX: use decH (total height of removed elements) not e->h() (boundary element, not removed)
            this->setScrollPos(this->scrollPos_ - decH);
            return true;
        }
        return false;
    }
